...
import pl.quicktask.app.scheduled.presentation.AddEditScheduledItemDialog
import pl.quicktask.app.scheduled.presentation.ScheduledListItem
import pl.quicktask.app.scheduled.presentation.ScheduledViewModel
...
fun ScheduledScreen(
    module: AppModule,
    onOpenDrawer: () -> Unit = {},
    viewModel: ScheduledViewModel = viewModel {
        ScheduledViewModel(module.items.scheduled, module.items.store)
    },
) {
    ...
    LaunchedEffect(uiState.showAddEditDialog, uiState.selectedItemToEdit?.itemId) {
    ...
    Scaffold(
        topBar = {
            AppTopBar(
                title = stringResource(Res.string.screen_scheduled),
        ...
        PullToRefreshBox(
            ...
            onRefresh = { viewModel.loadScheduledTasks(forceFetch = true) },
                        ...
                        LazyColumn(
                            ...
                            items(
                                ...
                                ScheduledListItem(
                                    ...
                                    onDelete = { viewModel.deleteScheduledTask(item.itemId) },
                                )
                            }
                        }
                    }
    ...
    if (uiState.showAddEditDialog) {
        AddEditScheduledItemDialog(
            itemToEdit = uiState.selectedItemToEdit,
            options = uiState.options,
            isSubmitting = uiState.isSubmitting,
            errorMessage = errorMessage,
            existingAttachments = uiState.selectedItemToEdit?.attachments
                ?.filterNot { it.attachmentId in removedAttachmentIds }
                ?: emptyList(),
            ...
            onConfirm = { title, note, scheduledAt, deferUntil, projectId, dueAt, contextIds, newContextNames, tagIds, newTagNames, recurrence ->
