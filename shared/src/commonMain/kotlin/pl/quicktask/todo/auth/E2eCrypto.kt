...
suspend fun wrapPrivateKey(
    ...
    val wrapKeyBytes = hkdf.secretDerivation(
        digest = SHA256,
        outputSize = 32.bytes,
        salt = ByteArray(0),
        info = PRIVATE_KEY_WRAP_INFO.encodeToByteArray(),
    ).deriveSecretToByteArray(exportKeyBytes)

    val aesGcm = provider.get(AES.GCM)
    val wrapKey = aesGcm.keyDecoder().decodeFromByteArray(AES.Key.Format.RAW, wrapKeyBytes)
...
suspend fun wrapItemKey(
    ...
    val wrapKeyBytes = hkdf.secretDerivation(
        ...
        outputSize = 32.bytes,
        salt = ByteArray(0),
        info = FILE_KEY_WRAP_INFO.encodeToByteArray(),
    ...
    val nonce = Random.nextBytes(NONCE_BYTES)
    val rawItemKey = itemKey.encodeToByteArray(AES.Key.Format.RAW)
    val encryptedPayload = wrapKey.cipher().encryptWithIv(
        iv = nonce,
        plaintext = rawItemKey,
    )

    val ephemeralPublicRaw = ephemeralKeyPair.publicKey.encodeToByteArray(EC.PublicKey.Format.RAW)
    val fullPayload = ephemeralPublicRaw + nonce + encryptedPayload
...
suspend fun unwrapItemKey(
    ...
    val nonce = payload.copyOfRange(
        EPHEMERAL_PUBLIC_BYTES,
        EPHEMERAL_PUBLIC_BYTES + NONCE_BYTES,
    )
    ...
    val rawItemKey = wrapKey.cipher().decryptWithIv(
        iv = nonce,
        ciphertext = ciphertext,
    )
    return aesGcm.keyDecoder().decodeFromByteArray(AES.Key.Format.RAW, rawItemKey)
}
